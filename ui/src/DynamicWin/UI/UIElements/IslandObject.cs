...
namespace DynamicWin.UI.UIElements
{
    public class IslandObject : UIObject
    {
        ...
        public bool IsActiveGlow = false;
        ...
        public override void Update(float deltaTime)
        {
            ...
            if (!hidden)
            {
                if (IsHovering || IsActiveGlow)
                {
                    ...
                    if (IsActiveGlow) currSize = new Vec2(220, 36);
                }
                else
                {
            ...
            float targetShadowStrength = (IsHovering || IsActiveGlow) ? 0.75f : 0.25f;
            float targetShadowSize = (IsHovering || IsActiveGlow) ? 35f : 7.5f;

            dropShadowStrength = Mathf.Lerp(dropShadowStrength, targetShadowStrength, 10f * deltaTime);
            dropShadowSize = Mathf.Lerp(dropShadowSize, targetShadowSize, 10f * deltaTime);

            Col targetBorderCol = MenuManager.Instance.ActiveMenu.IslandBorderColor();
            if (IsActiveGlow) targetBorderCol = new Col(255, 30, 66);

            borderCol = Col.Lerp(borderCol, targetBorderCol, 10f  * deltaTime);
        }
